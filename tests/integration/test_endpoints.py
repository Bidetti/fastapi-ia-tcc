...
class TestIAEndpoints:
    def test_process_image_detection(self, client, mock_ia_repository, mock_dynamo_repository):
        request_data = {
        ...
        with patch("src.modules.ia_integration.usecase.detect_usecase.DetectUseCase.execute", new_callable=AsyncMock) as mock_detect:
    ...
    def test_process_image_maturation(self, client, mock_ia_repository, mock_dynamo_repository):
        request_data = {
        ...
        with patch("src.modules.ia_integration.usecase.maturation_usecase.MaturationUseCase.execute", new_callable=AsyncMock) as mock_maturation:
            detection_result = DetectionResult(
                class_name="banana",
                confidence=0.95,
                bounding_box=[0.1, 0.1, 0.2, 0.2],
            ...
            assert len(response_json["results"]) == 1
            assert response_json["results"][0]["class"] == "banana"
    ...
    def test_process_image_error_handling(self, client):
        ...
        with patch("src.modules.ia_integration.usecase.detect_usecase.DetectUseCase.execute", 
