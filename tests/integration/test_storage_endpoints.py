...
class TestStorageEndpoints:
    ...
    def test_get_result_by_request_id(self, client, sample_detection_result):
        request_id = "test-request-id"

        with patch.object(
            GetResultUseCase, "get_by_request_id", new_callable=AsyncMock, return_value=sample_detection_result
        ):
            ...
            assert response_json["image_id"] == sample_detection_result.image_id
            assert response_json["model_type"] == sample_detection_result.model_type.value
            assert len(response_json["results"]) == 1
            assert response_json["results"][0]["class_name"] == "apple"
    ...
    def test_get_result_by_request_id_not_found(self, client):
        ...
        with patch.object(GetResultUseCase, "get_by_request_id", new_callable=AsyncMock, return_value=None):
            ...
            assert request_id in response_json["detail"]

    def test_get_results_by_image_id(self, client, sample_detection_result, sample_maturation_result):
        ...
        with patch.object(
            ...
            return_value=[sample_detection_result, sample_maturation_result],
        ):
            ...
            assert response_json[0]["model_type"] == "detection"
            assert response_json[1]["model_type"] == "maturation"
            assert response_json[0]["image_id"] == image_id
            assert response_json[1]["image_id"] == image_id
    ...
    def test_get_results_by_image_id_not_found(self, client):
        ...
        with patch.object(GetResultUseCase, "get_by_image_id", new_callable=AsyncMock, return_value=[]):
            ...
            response_json = response.json()
            assert "detail" in response_json
            assert image_id in response_json["detail"]

    def test_get_results_by_user_id(self, client, sample_detection_result, sample_maturation_result):
    ...
    def test_get_results_by_user_id_not_found(self, client):
        ...
        with patch.object(GetResultUseCase, "get_by_user_id", new_callable=AsyncMock, return_value=[]):
            ...
            response_json = response.json()
            assert "detail" in response_json
            assert user_id in response_json["detail"]
