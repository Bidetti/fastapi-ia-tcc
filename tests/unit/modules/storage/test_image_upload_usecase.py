...
class TestImageUploadUseCase:
    ...
    async def test_generate_presigned_url(self, mock_s3_repository, mock_dynamo_repository):
        ...
        mock_s3_repository.generate_image_key.return_value = "banana_quality_control/2025/05/12/banana-ripeness-uuid.jpg"
    ...
    async def test_upload_image(self, mock_s3_repository, mock_dynamo_repository):
        ...
        assert image.image_url == "https://fruit-analysis-bucket.s3.amazonaws.com/plantation_inspector/2025/05/12/banana-analysis-uuid.jpg"
    ...
    async def test_upload_image_with_s3_error(self, mock_dynamo_repository):
        ...
        mock_failing_s3_repository.generate_image_key = AsyncMock(return_value="distribution_center_qa/2025/05/12/banana-ripeness-uuid.jpg")
        mock_failing_s3_repository.upload_file = AsyncMock(side_effect=Exception("Falha no upload da imagem de maturação"))
    ...
    async def test_generate_presigned_url_with_error(self):
        ...
        mock_failing_s3_repository = AsyncMock()
        mock_failing_s3_repository.generate_image_key = AsyncMock(return_value="ripeness_analyst/2025/05/12/banana-maturation-uuid.jpg")
        mock_failing_s3_repository.generate_presigned_url = AsyncMock(side_effect=Exception("Falha na geração da URL para análise de maturação"))
