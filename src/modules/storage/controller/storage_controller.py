    ...
    try:
        ...
        if metadata:
            ...
            try:
                ...
                raise HTTPException(status_code=400, detail="Formato de metadata inválido. Deve ser JSON válido.")
...
async def get_result_by_request_id(
    ...
    try:
        ...
        if not result:
            raise HTTPException(status_code=404, detail=f"Resultado não encontrado para request_id: {request_id}")
...
async def get_results_by_image_id(
    ...
    try:
        ...
        if not results:
            raise HTTPException(status_code=404, detail=f"Nenhum resultado encontrado para image_id: {image_id}")
...
async def get_results_by_user_id(
    ...
    try:
        ...
        if not results:
            raise HTTPException(status_code=404, detail=f"Nenhum resultado encontrado para user_id: {user_id}")
