...
class DynamoRepository(DynamoRepositoryInterface):
    ...
    async def save_processing_result(self, result: ProcessingResult) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(
                f"Salvando resultado de processamento {result.request_id} no DynamoDB"
            )
            return await self.dynamo_client.put_item(item)

        except Exception as e:
            logger.exception(
                f"Erro ao salvar resultado de processamento no DynamoDB: {e}"
            )
            raise
    ...
    async def get_result_by_request_id(
        ...
        try:
            ...
            if not items or len(items) == 0:
                logger.warning(
                    f"Resultado não encontrado para request_id: {request_id}"
                )
                return None
            ...
            logger.exception(
                f"Erro ao recuperar resultado por request_id do DynamoDB: {e}"
            )
            raise
    ...
    async def get_results_by_image_id(self, image_id: str) -> List[ProcessingResult]:
        ...
        try:
            items = await self.dynamo_client.query_items(
                key_name="pk", key_value=f"IMG#{image_id}"
            )
            ...
            logger.exception(
                f"Erro ao recuperar resultados por image_id do DynamoDB: {e}"
            )
            raise
    ...
    async def get_results_by_user_id(
        ...
        try:
            ...
            logger.exception(
                f"Erro ao recuperar resultados por user_id do DynamoDB: {e}"
            )

    async def save_combined_result(
        self, combined_result: CombinedResult
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"Salvando resultado combinado para imagem {combined_result.image_id} no DynamoDB"
            )
            return await self.dynamo_client.put_item(item)
    ...
    async def get_combined_result(self, image_id: str) -> Optional[CombinedResult]:
        ...
        try:
            ...
            if not item:
                logger.warning(
                    f"Resultado combinado não encontrado para image_id: {image_id}"
                )
                return None
